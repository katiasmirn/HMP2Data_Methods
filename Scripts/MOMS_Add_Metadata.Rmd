...
check <- meta_sub[newmoms %in% newmer$SAMPLE_ID,]
new_meta <- merge(x = meta_sub, y = newmer, by.x = 0, by.y = "SAMPLE_ID")
na.test(new_meta)

#In health history, some samples are labeled twice but have two different inputs. For instance, EP081373 has two different entries with one saying 
#the subject is African American and the other one saying they are not.
...
new_data <- new_meta %>% distinct(Row.names, .keep_all = TRUE) #keeps only the first unique row name, done so that it can merge correctly with phyloseq 
#object
