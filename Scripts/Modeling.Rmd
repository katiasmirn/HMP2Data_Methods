...
full_set$p_Snea <- full_set$Sneathia_amnii/full_set$num_reads
full_set$p_Prev <- full_set$Prevotella_cluster2/full_set$num_reads
full_set$p_TM <- full_set$`TM7_OTU-H1`/full_set$num_reads
...
#prop_plot
iprop_plot <- ggplot(full_set, aes(p_Snea)) + geom_histogram() 
#iprop_plot

pprop_plot <- ggplot(full_set, aes(p_Prev)) + geom_histogram() 
#pprop_plot

tprop_plot <- ggplot(full_set, aes(p_TM)) + geom_histogram()
#tprop_plot

bprop_plot <- ggplot(full_set, aes(p_BVAB)) + geom_histogram()
#bprop_plot
#stratify by race
ggarrange(prop_plot, iprop_plot, tprop_plot, pprop_plot, bprop_plot, labels = c("Iners", "Sneathia", "TM7", "Prevotella", "BVAB1"), ncol = 3, nrow = 2)
...
race_prop_plot
irace_prop_plot <- ggplot(full_set, aes(p_Snea)) + geom_histogram(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("Sneathia Proportion Histogram")
irace_prop_plot
#stratify by race
prace_prop_plot <- ggplot(full_set, aes(p_Prev)) + geom_histogram(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("Prevotella Proportion Histogram")
prace_prop_plot
trace_prop_plot <- ggplot(full_set, aes(p_TM)) + geom_histogram(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("TM7 Proportion Histogram")
trace_prop_plot
brace_prop_plot <- ggplot(full_set, aes(p_BVAB)) + geom_histogram(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("BVAB Proportion Histogram")
brace_prop_plot
```
...
mytypes <- as.factor(mytypes)f
full_set <- full_set %>%
  mutate(vagitype=mytypes)

full_set$class_Iners <- as.factor(ifelse(full_set$p_Iners > 0.3, "High", "Low"))
full_set$class_Snea <- as.factor(ifelse(full_set$p_Snea > 0.3, "High", "Low"))
full_set$class_Prev <- as.factor(ifelse(full_set$p_Prev > 0.3, "High", "Low"))
full_set$class_TM <- as.factor(ifelse(full_set$p_TM > 0.3, "High", "Low"))
full_set$class_BVAB <- as.factor(ifelse(full_set$p_BVAB > 0.3, "High", "Low"))
```
...
snea_plot <- ggplot(full_set, aes(class_Snea)) + geom_bar() 
tm_plot <- ggplot(full_set, aes(class_TM)) + geom_bar() 
prev_plot <- ggplot(full_set, aes(class_Prev)) + geom_bar() 
bvab_plot <- ggplot(full_set, aes(class_BVAB)) + geom_bar()

ggarrange(iners_plot, snea_plot, tm_plot, prev_plot, bvab_plot, labels = c("Iners", "Sneathia", "TM7", "Prevotella", "BVAB1"), ncol = 3, nrow = 2)
```

```{r}
#Distribution of High and Low (do it across race)
irace_prop_plot <- ggplot(full_set, aes(class_Iners)) + geom_bar(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("Iners Dichotomized Histogram")
irace_prop_plot

#Distribution of High and Low (do it across race)
srace_prop_plot <- ggplot(full_set, aes(class_Snea)) + geom_bar(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("Sneathia Dichotomized Histogram")
srace_prop_plot

#Distribution of High and Low (do it across race)
prace_prop_plot <- ggplot(full_set, aes(class_Prev)) + geom_bar(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("Prevotella Dichotomized Histogram")
prace_prop_plot

#Distribution of High and Low (do it across race)
trace_prop_plot <- ggplot(full_set, aes(class_TM)) + geom_bar(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("TM7 Dichotomized Histogram")
trace_prop_plot

#Distribution of High and Low (do it across race)
brace_prop_plot <- ggplot(full_set, aes(class_BVAB)) + geom_bar(aes(group=race, color=race)) + facet_wrap(~race, ncol=3) + ggtitle("BVAB Dichotomized Histogram")
brace_prop_plot
```
